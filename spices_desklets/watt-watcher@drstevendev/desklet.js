...
MyDesklet.prototype = {
    ...
    _init: function(metadata, desklet_id) {
        ...
        this.powerSourceId = "BAT0";

        // initialize settings
        this.settings = new Settings.DeskletSettings(this, this.metadata["uuid"], desklet_id);
        this.settings.bindProperty(Settings.BindingDirection.IN, "refresh-interval", "refresh_interval", this._on_setting_changed);
        ...
        this.settings.bindProperty(Settings.BindingDirection.IN, "background-color", "backgroundColor", this._on_setting_changed);
        this.settings.bindProperty(Settings.BindingDirection.IN, "border-color", "borderColor", this._on_setting_changed);
    ...
    _updateFilePaths: function() {
        this.capacityFile = Gio.file_new_for_path(`/sys/class/power_supply/${this.powerSourceId}/capacity`);
        this.fullEnergyFile = Gio.file_new_for_path(`/sys/class/power_supply/${this.powerSourceId}/charge_full`);
        this.isChargingFile = Gio.file_new_for_path("/sys/class/power_supply/AC/online");
        this.powerNowFile = Gio.file_new_for_path(`/sys/class/power_supply/${this.powerSourceId}/charge_now`);
        this.currentNowFile = Gio.file_new_for_path(`/sys/class/power_supply/${this.powerSourceId}/current_now`);
        this.voltageNowFile = Gio.file_new_for_path(`/sys/class/power_supply/${this.powerSourceId}/voltage_now`);
    ...
    _setupUI: function() {
        ...
        // Header Text
        this.headerLabel = new St.Label({ text: `${this.headerText}: ${this.powerSourceId}`, style_class: "header-bar", });
        this.mainBox.add(this.headerLabel);

        this.canvas = new St.DrawingArea({ style_class: "drawing-area", reactive: false });
    ...
    setAllDataValues: function() {
        ...
        if (this.showStatusBar) {
            ...
            let rate_watt = parseInt(this.currentNow * this.voltageNow);
            const rateLabel = new St.Label({
                text: `${chargeState}: ${this.currentNow}mA*${this.voltageNow}V=${rate_watt}mWh,${this.powerNow} mWh`,
                style: `font-size: 13px; margin-right: 12px;`,
            });
            this.statusBar.add(rateLabel);
            const totalCapacityLabel = new St.Label({
                text: `FCC: ${this.energyFull} mWh`,
                style: `font-size: 13px; margin-right: 12px;`,
            });
    ...
    setTimeLeftInHoursString: function(maxCapacity, capacityPrct, rate) {
        //let prct = rate / maxCapacity;
        //this.timeLeftString = `(${prct})`;
    ...
    setEnergyFull: function() {
        this.fullEnergyFile.load_contents_async(null, (file, response) => {
            ...
            if (success) {
                ...
                this.energyFull = (energyFull) / 1000;
            }
        });
    },
    ...
    setCurrentNow: function() {
        this.currentNowFile.load_contents_async(null, (file, response) => {
            let [success, contents, tag] = file.load_contents_finish(response);
            if (success) {
                let powerValue = ByteArray.toString(contents);
                this.currentNow = (powerValue) / 1000;
                //this.powerNow = this.powerNow.toFixed(1);
            }
            GLib.free(contents);
        });
    ...
    setVoltageNow: function() {
        this.voltageNowFile.load_contents_async(null, (file, response) => {
            let [success, contents, tag] = file.load_contents_finish(response);
            if (success) {
                let powerValue = ByteArray.toString(contents);
                this.voltageNow = (powerValue) / 1000000;
                //this.powerNow = this.powerNow.toFixed(1);
            }
            GLib.free(contents);
        });
